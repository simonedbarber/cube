...
the `CROSS JOIN` clause or via `__cubeJoinField`. The SQL API also exposes
`__cubeExplicitJoinField`, a direction-strict variant that lets you traverse
a declared join in either direction without declaring it twice. Reverse-
direction traversal in views' `join_path` works under the same opt-in flag
(see the [SQL API joins reference][ref-sql-api-joins] for examples and
pre-aggregation interaction). In any case, Cube will analyze the query and
follow provided join hints.
