...
If `true`, enables bidirectional SQL joins. The planner may synthesize a
reverse `JoinEdge` from a declared edge for two narrowly documented entry
points only: the [`__cubeExplicitJoinField`][ref-sql-api-joins] virtual
column in the SQL API, and views whose [`join_path`][ref-data-model-joins]
traverses the declared edge in the opposite direction. All other graph
traversals (REST `joinHints`, `__cubeJoinField`, member resolution, `/meta`
export, connectedness analysis, pre-aggregation matching) continue to use
the strictly directed graph defined by the data model regardless of this
flag.
...
defined for one direction will not be served for a query that requests the
reverse direction — the planner falls back to the live join with the
synthesized reverse edge. Define matching pre-aggregations per direction
if both need acceleration.
...
[ref-sql-api-joins]: /product/apis-integrations/core-data-apis/sql-api/joins
[ref-data-model-joins]: /product/data-modeling/concepts/working-with-joins
